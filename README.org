...
Las imágenes de  ~agarbuno/bayesiana~ (puedes ver imágenes en mi hub de Docker  [[https://hub.docker.com/repository/docker/agarbuno/bayesiana][aqui]], y/o los ~Dockerfiles~ [[https://github.com/agarbuno/dockerfiles][aqui]]) permiten tener un ambiente para trabajar con ~Docker~ en nuestro curso. Actualmente hay dos /tags/ para trabajar con el ambiente.
1. El /tag/ ~2022~ ó ~latest~, basado en una distribución linux en una emulación con un procesador ~Intel~.
2. El /tag/ ~amd~, basado en una distribución linux en una emulación con un procesador ~AMD~.

#+REVEAL: split
Ambos proveen de un ambiente donde se podrán instalar las paqueterías que utilizaremos sin ningún problema$\dagger$. La idea, adicional, es que utilicemos /volumes/ para poder tener permanencia en nuestro trabajo.
...
*** Observaciones.
La imagen ~amd~ puede emular al instalación de ~R~ sin embargo hay dos inconvenientes: 1) la imagen es mas pesada que la basada en ~Intel~ y 2) la versión de ~R~ es la ~4.0.3~ lo cual quiere decir que *no podrá* utilizar el /pipe/ base sino el de ~maggritr~: ~%>%~. 

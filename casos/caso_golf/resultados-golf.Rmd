...
La velocidad final de la pelota en términos de la velocidad inicial, usando
la aceleración en el *green,* es
...
pie (30.48 cm), sin embargo la fuerza tiene un error multiplicativo. Esto es, 
...
data_new <- list(x = datos$x, n = datos$n, y = datos$y, J = nrow(datos), 
                 ...
                 overshot = 30.48, distance_tolerance = 145)
```
...
Al explorar los residuales encontramos que parece haber cierto patrón. 
...
medias <- muestras %>% 
  ...
  group_by(parameters) %>% 
  summarise(media = mean(residuals), .groups = 'drop') %>% 
...
Podríamos ajustar sólo en los datos nuevos. Pero no tenemos alguna justificación
específica para descartar los que ya teníamos. Por lo pronto usaremos ambos
conjuntos sin distinción.

Una forma que tenemos es incorporar una aproximación continua a las proporciones
reportadas, misma que podemos utilizar para incorporar error de medición. El 
modelo en `Stan` no cambia mucho y se vuelve un poco mas flexible. 
...
Podemos incorporar información *débil* en los parametros de escala, eso es por
medio de normales truncadas en la región positiva. 
...
datos %>% 
  ...
  ggplot(aes(x = x, y = media)) + 
    ...
    ylab('Residuales del modelo ajustado') + 
    xlab('Distancia (cm)') + 
    ggtitle("Modelo con angulo, fuerza de tiro, y error de medición")
```
...
datos.
- Faltan barras de dispersion en los graficos de residuales. 

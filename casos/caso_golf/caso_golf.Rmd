...
```{r}
construir_curvas <- function(fit = fit){
  sigmas <- rstan::extract(fit)$sigma
  exitos <- rstan::extract(fit)$exitos %>% as_tibble %>% 
    rownames_to_column(var = "rep") %>% 
    mutate(rep = as.numeric(rep)) %>% 
    mutate(sigma = sigmas)
  nombres <- colnames(exitos %>% select(-rep, -sigma))
  distancias <- tibble(name = nombres, dist = sim_data$x)
  exitos <- exitos %>% 
    pivot_longer(cols = V1:V7) %>% 
    left_join(tibble(n_intentos = sim_data$n, name = nombres)) %>%
...
fit <- stan(file = "stan/simular_ensemble.stan",
            data = sim_data,
            iter = R, warmup = 0, chains = 1, refresh = R,
            seed = 432, algorithm = "Fixed_param")
```

## Verificación a priori

En rimer lugar, la media de número de éxitos se ve razonable:
...
fit_error_1 <- stan(file = "stan/simular_ensemble.stan",
            data = sim_data_error_1,
            iter = R, warmup = 0, chains = 1, refresh = R,
            seed = 43552, algorithm = "Fixed_param")
exitos_error_1 <- construir_curvas(fit_error_1)
...
exitos_obs <- rstan::extract(fit)$exitos
sigma <- rstan::extract(fit)$sigma
modelo <- stan_model(file = "stan/ajuste_datos.stan")
ajustar_modelo <- function(rep, exitos_obs, sigma, modelo, data){
  ...
  ajuste <- sampling(modelo, data = datos_stan_1, seed = 21053,
                     verbose = FALSE,
                     iter = 2000, warmup = 1000,
                     show_messages = FALSE, refresh = 0)
  diagnosticos <- get_num_divergent(ajuste)
  if(diagnosticos > 0){
    print(datos_stan_1)
    print(sigma)
  }
  # para calibración
  sigma_sim <- extract(fit)$sigma %>% thin(., k = 10)
  sbc_rank <- mean(sigma < sigma_sim)
  resumen <- summary(ajuste)$summary
...
```{r}
ajustes <- map(1:200, 
               ~ ajustar_modelo(.x, exitos_obs[.x, ], sigma[.x], modelo, sim_data)) %>% 
  bind_rows()
...
calib_inf <- ajustes %>% 
  mutate(post_media_sigma = map_dbl(resumen, ~.x["sigma", "mean"]),
         post_sd_sigma = map_dbl(resumen, ~ .x["sigma", "sd"])) %>% 
...
datos_stan <- list(p = nrow(datos), x = as.integer(datos$x), 
                   ...
                   exitos_obs = datos$y)
modelo <- stan_model(file = "stan/ajuste_datos.stan")
ajuste <- sampling(modelo, data = datos_stan, seed = 2023,
                     iter = 4000, warmup = 2000)
```

```{r}
summary(ajuste)$summary
#(summary(ajuste)$summary[1,] * 180/pi) %>% round(2)
```
...
```{r}
sigmas <- rstan::extract(ajuste)$sigma
...
```{r}
sigmas <- (180/pi) * rstan::extract(ajuste)$sigma
qplot(sigmas)
```

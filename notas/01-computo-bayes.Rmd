...
plot_dist <- ggplot(caminata, aes(x = isla)) +
    ...
    labs(title = "Distribución objetivo", 
       y = expression(pi(theta))) + sin_lineas
...
$$\alpha(u^{(n)}, u^{(\star)}) = \min \bigg\{ \frac{\pi(u^{(n)})}{\pi(u^{(\star)}) } \cdot \frac{q(u^{(n)}, u^{(\star)})}{q(u^{(\star)}, u^{(n)})}, \, 1\bigg\}.$$
...
g_1 <- ggplot(sims_metro_tbl, aes(x = theta)) + 
  geom_histogram(aes(y = ..density..)) +
  labs(subtitle = "Metropolis-Hastings")+ sin_lineas
g_2 <- ggplot(sims_indep_tbl, aes(x = theta)) + 
  geom_histogram(aes(y = ..density..)) +
  labs(subtitle = "rbeta")+ sin_lineas
g_1 + g_2
```
...
distribución objetivo ésta se mantiene en la región adecuada. Pro otro lado,
...
esperaríamos que nuestra cadena se distirbuya de acuerdo a la distribución que
nos interesa.
...
donde las funciones $K$ y $V$ pueden ser interpretadas como las funciones de
...
por medio del sistema de ecuaciones de movimiento 

$$ \frac{\text{d}\theta}{\text{d}t} = \frac{\partial H}{\partial \vartheta}, \qquad \frac{\text{d}\vartheta}{\text{d}t} = -\frac{\partial H}{\partial \theta}, $$

lo cual pone en evidencia que es un sistema que conserva la energía y que
interpretamos como un sistema de evolución de particulas que se "mantiene"
dentro de las curvas de nivel. Lo cual es de suma importancia pues 
quisiéramos que, para un nivel de inercia dado $\vartheta^\star$, la trayectoria
del sistema $(\theta, \vartheta^\star)$ se mantenga unfirmomente dentro de la
curva $H(\theta, \vartheta^\star),$ pues esto representa uniformidad dentro de
la curva dictada por $\pi(\theta).$

El sistema de ecuaciones Hamiltonianas se resuelve en un tiempo discreto
ficticio. Cualquier patología que se encuentre en este simulación determinista
puede indicar problemas con el modelo $\pi(\theta),$ lo cual veremos mas
adelante. El componenente aleatorio viene de generar de manera independiente el 
componente de inercia $\vartheta$ de la distribución $\pi(\vartheta|\theta).$
Ésta última se considera por simplicidad como una variable aleatoria Gaussiana. 
Las derivadas parciales en el sistema Hamiltoniano se encargan de incorporar 
la complejidad adecuada de las trayectorias. 

HMC es computacionalmente más costoso que Metrópolis o Gibbs, sin embargo, sus
...
tiempo de simulación en cada paso del algoritmo. El método derivado de HMC que
utiliza `Stan` se conoce como *No U-Turn Sampler* [@nuts; @stan].

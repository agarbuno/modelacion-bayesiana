...
trayectoria. El sistema de evolución de particulas "mantiene" las curvas de
nivel. Lo cual es de suma importancia pues
...
ficticio. Usualmente por un método mixto (Euler implícito e explicíto en medios
pasos). Puedes consultar [@nealHMC] para mayores detalles en la implementación.
Cualquier patología que se encuentre en este simulación determinista
puede indicar problemas con el modelo $\pi(\theta),$ esto lo veremos más
...
$$\vartheta \, | \, \theta \sim \mathsf{N}(0, M^{-1}).$$
 
Por otro lado, las derivadas parciales en el sistema Hamiltoniano se encargan de
incorporar la complejidad adecuada de las trayectorias.
...
Escribe el cociente de Metropolis-Hastings para HMC. Considera que es un sistema 
"bivariado". ¿Será realmente necesario incorporarlo cuando simulamos de un sistema 
Hamiltoniano para inferencia Bayesiana?

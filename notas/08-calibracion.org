...
- [[#cbs-en-stan][CBS en Stan]]
  ...
  - [[#ejemplo][Ejemplo]]
  - [[#ejemplo-escuelas-reparametrizado][Ejemplo: Escuelas reparametrizado]]
...
#+begin_src R :file images/escuelas-sbc-histogramas-diff.jpeg  :exports results :results output graphics file :eval never 
  resultados.escuelas |>
    unnest(results) |>
    pivot_longer(cols = 2:4) |>
    mutate(bins = cut(value, breaks = seq(0,1000, length.out= 21))) |>
    group_by(name, bins) |>
    tally() |>
    filter(!is.na(bins)) |>
    mutate(ecdf = cumsum(n)/sum(n),
           ...
           rank = seq(25, 1000, 50),
           diff.cdf = ecdf - cdf,
           diff.lo  = - 2 * sqrt(rank/1000 * (1 - rank/1000)/n_reps),
           diff.hi  = + 2 * sqrt(rank/1000 * (1 - rank/1000)/n_reps), 
           ) |>
    ggplot(aes(x = rank)) +
    geom_hline(yintercept = 0, lty = 2, color = "gray30") + 
    geom_stepribbon(aes(ymin = diff.lo, ymax = diff.hi), fill = "grey70", alpha = .3) +
...
** Ejemplo: Escuelas reparametrizado

#+begin_src stan :tangle ../modelos/calibracion/escuelas-reparam.stan
  transformed data {
    real mu_sim = normal_rng(0, 5);
    real tau_sim = fabs(normal_rng(0, 5));
    int<lower=0> J = 8;
...
#+begin_src R :exports none :results none
  modelos_files <- "modelos/compilados/calibracion"
  ruta <- file.path("modelos/calibracion/escuelas-reparam.stan")
  modelo.gp <- cmdstan_model(ruta, dir = modelos_files)
#+end_src
...
#+begin_src R :exports none :results none :eval never 
  ...
  ## Cuidado en correr (paciencia)
  resultados.escuelas.rp <- tibble(id = 1:n_reps) |>
    mutate(results = map(id, crea_muestras, modelo.gp))
#+end_src


#+HEADER: :width 1200 :height 400 :R-dev-args bg="transparent"
#+begin_src R :file images/escuelas-sbc-histograms-reparam.jpeg :exports results :results output graphics file :eval never
  resultados.escuelas.rp |>
    unnest(results) |>
    pivot_longer(cols = 2:4) |>
    ggplot(aes(x = value)) +
    geom_hline(yintercept = n_reps/n_ranks, lty = 2, color = 'black') +
...
#+RESULTS:
[[file:../images/escuelas-sbc-histograms-reparam.jpeg]]

#+HEADER: :width 1200 :height 400  :R-dev-args bg="transparent"
#+begin_src R :file images/escuelas-sbc-histogramas-diff-reparam.jpeg  :exports results :results output graphics file :eval never
  resultados.escuelas.rp |>
    unnest(results) |>
    pivot_longer(cols = 2:4) |>
    mutate(bins = cut(value, breaks = seq(0,1000, length.out= 21))) |>
...
#+RESULTS:
[[file:../images/escuelas-sbc-histogramas-diff-reparam.jpeg]]

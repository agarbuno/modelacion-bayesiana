...
*Objetivo*. Estudiaremos diagnósticos típicos de métodos de simulación Markoviana
 (por Metropolis-Hastings o HMC) con el objetivo de poder identificar problemas
 y posibles soluciones para simulaciones deficientes. \\
*Lectura recomendada*: Para una revisión de los diagnósticos de MCMC busca la
 ...
 diagnósticos. Además, se mencionan algunas técnicas modernas de diagnóstico
 como los métodos basados en los principios de Stein o distancias adecuadas para
...
\begin{align}
\mathsf{SE}({\pi}(f)) \approx \left(  \frac{\hat{\mathbb{V}}_\pi(f)}{\widehat{\mathsf{ESS}}} \right)^{\frac12}\,.
\end{align}

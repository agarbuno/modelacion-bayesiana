...
\begin{align}
\mathbb{E}\left[ \hat{\pi}_N^{\textsf{MC}}(f) - \pi(f)\right] = 0\,,
\end{align}
...
\begin{align*}
\sup_{f \in \mathcal{F}} \, \,  \mathbb{E}\left[ \left(\hat{\pi}_N^{\textsf{MC}}(f) - \pi(f) \right)^2 \right] \leq \frac1N\,.
\end{align*}

#+REVEAL: split
En particular, la varianza del estimador (*error estándar*) satisface la igualdad

$$ \textsf{ee}^2\left(\hat{\pi}_N^{\textsf{MC}}(f)\right) = \frac{\mathbb{V}_\pi( f )}{N}\,.$$
...
\begin{align}
\sqrt{N} \left(\hat{\pi}_N^{\textsf{MC}} (f) - \pi(f) \right) \sim \mathsf{N}\left(0, \mathbb{V}_\pi(f)\right)\,.
\end{align}
